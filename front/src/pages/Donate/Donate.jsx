...
const Donate = () => {
  const [form, setForm] = useState({ name: '', email: '', phone: '', transactionId: '' });
  const [paymentSettings, setPaymentSettings] = useState({ upiId: '', merchantName: '', amount: '500' });
  ...
  const fetchPaymentSettings = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setSelectedAmount(QUICK_AMOUNTS.includes(Number(settings.amount)) ? String(settings.amount) : '500');
      }
    } catch (err) {
  ...
  const upiPayload = useMemo(() => {
    ...
    const merchant = encodeURIComponent(paymentSettings.merchantName || 'ITF OF INDIA');
    return `upi://pay?pa=${paymentSettings.upiId}&pn=${merchant}&am=${numericAmount}&cu=INR`;
  }, [paymentSettings.upiId, paymentSettings.merchantName, numericAmount, validAmount]);

  const handleDonate = async (e) => {
    e.preventDefault();
    setError(null);
    if (!validAmount) return setError('Please enter a valid amount');
    if (!paymentSettings.upiId) return setError('Payment gateway is not configured. Please try again later.');
    if (!form.name.trim()) return setError('Name is required');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) return setError('Please enter a valid email address');
    if (!/^[6-9]\d{9}$/.test(form.phone.trim())) return setError('Please enter a valid 10-digit mobile number');
    setLoading(true);
    try {
  ...
  if (receipt) {
    const donorName = receipt.name && receipt.name.trim() ? receipt.name : 'Supporter';
    return (
      <div className="donation-confirm">
        <div className="donation-confirm-card">
          ...
          <p className="confirm-subtitle">
            Your contribution has been received and is currently under verification by our team.
          </p>
          ...
          <blockquote className="donation-quote">
            "No one has ever become poor by giving. Every contribution creates hope."
          </blockquote>
  ...
  return (
    <div className="donation-page">
      <div className="donation-card">
        ...
        <p>Use the same UPI used for player registration. Select amount to generate QR instantly.</p>
        ...
        <div className="qr-block">
          {paymentLoading ? (
            ...
            <QRCodeSVG value={upiPayload} size={220} includeMargin={true} level="H" />
          ) : (
        ...
        <div className="upi-info">
          <span>UPI ID:</span> <strong>{paymentSettings.upiId || 'Not configured'}</strong>
        </div>
        ...
        <form className="donation-form" onSubmit={handleDonate}>
          ...
          <label>Phone</label>
          <input type="text" name="phone" value={form.phone} onChange={handleChange} placeholder="10-digit mobile number" required maxLength={10} inputMode="numeric" pattern="[6-9][0-9]{9}" />

          <label>Transaction ID / UTR (optional)</label>
          <input type="text" name="transactionId" value={form.transactionId} onChange={handleChange} placeholder="Enter UTR or transaction ID" />

          {error && <div className="donation-error">{error}</div>}

          <button className="btn-donate" type="submit" disabled={loading || !validAmount || paymentLoading || !paymentSettings.upiId}>{loading ? 'Processing...' : 'I Have Paid - Generate Receipt'}</button>
        </form>
      </div>
    </div>
  );
